...
import br.com.codehive.projetogeral.projetos.todoit.service.TagsService;
...
public class TagsController {
    ...
    public ResponseEntity<?> listar(
        ...
        return ResponseEntity.ok(service.listarTags(idUsuario));
    }

    @PostMapping("/adicionar")
    public ResponseEntity<?> adicionar(
            @RequestHeader("Authorization") String authorizationHeader,
            @NonNull @RequestBody TagsModel tagsModel
    ){
        RetorneToken retorneToken = UtilidadeGerais.validaToken(authorizationHeader);
        if(!retorneToken.isValidacao()) return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(retorneToken.getResposta());
        var retu = service.adicionarTag(tagsModel);
